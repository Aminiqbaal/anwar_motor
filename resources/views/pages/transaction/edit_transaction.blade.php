...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-footer">
            <select id="mechanic_id" class="form-control">
                ...
                <option value="{{ $mechanic->id }}" {{ $transaction->mechanic_id == $mechanic->id ? 'selected' : '' }}>{{ $mechanic->data->name }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="card-footer d-flex">
        <form action="/transaction/{{ $transaction->id }}" method="POST" class="form-inline">
            @csrf
            @method('PUT')
            <input type="hidden" name="grand_total" value="{{ $transaction->grand_total }}">
            <input type="hidden" name="mechanic_id" value="{{ $transaction->mechanic_id }}">
            ...
            <label for="created_at">Tanggal Transaksi</label>&nbsp;<input type="date" class="form-control" name="created_at" value="{{ $transaction->created_at }}" required>
            <select name="customer_id" class="form-control mx-3" required>
                <option value="" hidden>Nama Pelanggan</option>
                @foreach ($customers as $customer)
                <option value="{{ $customer->id }}" {{ old('customer_id') == $customer->id || $transaction->customer_id == $customer->id ? 'selected':'' }}>{{ $customer->name . ' (' . $customer->phone . ')' }}</option>
                @endforeach
            </select>
